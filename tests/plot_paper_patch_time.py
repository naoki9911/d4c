...
ax.set_ylabel("Time to mount bundles (Seconds)")
...
for i, l in zip(range(0, data_num), labels):
    ...
    ax.bar([(x*factor + (int(x/2)*BAR_WIDTH/3))+(BAR_WIDTH*i) for x in range(0, len(keys))], value, align="edge",  edgecolor="black", linewidth=1, width=BAR_WIDTH, label=l[1])
...
xticks_labels.extend(["postgres", "nginx", "redis", "redis"])
