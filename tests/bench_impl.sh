...
for ((i=0; i < $(expr ${#IMAGE_VERSIONS[@]} - 1); i++));do
	...
	for ((j=0; j < $RUN_NUM; j++));do
		...
		if [ $j -eq 0 ]; then
			...
			$BIN_CTR_CLI --labels $LABELS,old:$LOWER,new:$UPPER,mode:binary-diff,out:$LOWER-$UPPER stat diff --benchmark --pathALabel native --pathBLabel di3fs --count 2 $UPPER /tmp/fuse
		fi
		fusermount3 -u /tmp/fuse
	done
